...
pub trait RenderEventHandler<CommandBuffer> {
    fn on_resize(&mut self, ctx: &VulkanoContext, window: Arc<Window>) -> Result<()>;
    fn on_render(&mut self) -> Result<Vec<Arc<CommandBuffer>>>;
}

pub struct WindowEventHandler<RenderHandler: RenderEventHandler<PrimaryAutoCommandBuffer>> {
    window: Arc<Window>,
    ctx: VulkanoContext,
...
impl<RenderHandler: RenderEventHandler<PrimaryAutoCommandBuffer> + 'static>
    WindowEventHandler<RenderHandler>
{
    ...
    fn handle_acquired_image(
        ...
        self.fences[image_idx as usize] = previous_future
            .join(acquire_future)
            .then_execute(
                self.ctx.queue(),
    ...
    fn run_inner(&mut self, event: Event<()>, control_flow: &mut ControlFlow) -> Result<()> {
        match event {
            ...
            Event::MainEventsCleared => {
                ...
                match swapchain::acquire_next_image(self.ctx.swapchain(), None)
                    .map_err(Validated::unwrap)
                {
